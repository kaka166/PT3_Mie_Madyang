...
import { Banknote, Receipt, Wallet, SlidersHorizontal, ChevronDown, Search, User, Printer, FileSpreadsheet } from "lucide-react";
...
import { PeriodFilter, DateRange, PERIOD_PRESETS } from "@/components/common/PeriodFilter";

type FilterPeriod = "Harian" | "Mingguan" | "Bulanan" | "Tahunan";

function FilterDropdown({ value, onChange }: { value: FilterPeriod; onChange: (v: FilterPeriod) => void }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-40 bg-white rounded-xl border border-neutral-200 shadow-lg z-20 py-1">
          {options.map(opt => (
...
export default function LaporanPemasukan() {
  ...
  const filteredData = data.filter(item => {
    const matchSearch = !search || item.nama?.toLowerCase().includes(search.toLowerCase()) || item.no?.includes(search);
    const matchKasir = !kasirFilter || item.kasir?.toLowerCase().includes(kasirFilter.toLowerCase());
  ...
  const getGroupKey = (waktu: string) => {
    ...
    if (rekapFilter === "Harian") return d.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" });
    if (rekapFilter === "Mingguan") {
      const start = new Date(d); start.setDate(d.getDate() - d.getDay());
      const end = new Date(start); end.setDate(start.getDate() + 6);
      return formatTanggalRange(start.toISOString().slice(0,10), end.toISOString().slice(0,10));
    }
    if (rekapFilter === "Bulanan") return d.toLocaleDateString("id-ID", { month: "long", year: "numeric" });
    return d.getFullYear().toString();
  };

  const groupedRekap = Object.values(
    data.reduce((acc: Record<string, { rentang: string; total: number; transaksi: number }>, item) => {
  ...
  const totalPemasukan = data.reduce((acc, i) => acc + Number(i.jumlah || 0), 0);
  ...
  const exportCSV = () => {
    const headers = ["No. Transaksi", "Nama", "Waktu", "Kasir", "Metode", "Kondisi", "Jumlah"];
    const rows = filteredData.map(item => [
      item.no || "", item.nama || "",
      item.waktu ? formatTanggal(item.waktu) : "",
      item.kasir || "", item.metode || "", item.kondisi || "", item.jumlah || 0,
    ]);
    ...
    <div className="h-full overflow-y-auto min-h-screen bg-neutral-100 p-4 md:p-8 font-sans pb-24">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex flex-wrap justify-between items-start gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[#F53E1B]">Laporan Penjualan</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            { title: "RATA-RATA PER TRANSAKSI", value: formatRupiah(rataRata), icon: Wallet },
          ].map((m, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 border border-neutral-100 shadow-sm flex flex-col justify-between min-h-[120px] hover:shadow-md transition-all">
              <div className="p-2.5 bg-blue-50 text-blue-500 rounded-xl w-fit"><m.icon size={20} /></div>
              <div>
                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">{m.title}</p>
                <p className="text-2xl font-extrabold text-neutral-800">{loading ? "..." : m.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm overflow-hidden">
          <div className="p-5 flex flex-wrap justify-between items-center border-b gap-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={3} className="py-8 text-center text-gray-400">Memuat...</td></tr>
        ...
        <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm overflow-visible">
          <div className="p-5 flex flex-wrap justify-between items-center border-b gap-3 relative z-20">
            <h2 className="text-lg font-bold text-neutral-900">Detail Pemasukan</h2>
            <div className="flex flex-wrap gap-2 items-center">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="py-10 text-center text-gray-400">Belum ada data penjualan</td></tr>
                ) : paginatedData.map((row, idx) => (
                  <tr key={idx} onClick={() => setSelectedRow(row)}
                    className={`cursor-pointer ${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-red-50 transition`}>
                    <td className="px-5 py-3.5 font-semibold text-neutral-700">{row.no}</td>
          ...
          <div className="p-4 border-t flex justify-between items-center text-sm text-gray-500">
            <span>Showing {filteredData.length === 0 ? 0 : startIndex + 1}–{Math.min(startIndex + itemsPerPage, filteredData.length)} of {filteredData.length}</span>
        ...
        {selectedRow && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl p-6 relative">
              <button onClick={() => setSelectedRow(null)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500">✕</button>
              <h2 className="text-xl font-bold mb-4">Detail Transaksi {selectedRow.no}</h2>
              <div className="space-y-2 text-sm mb-4">
                <p><b>Customer:</b> {selectedRow.nama}</p>
                <p><b>Waktu:</b> {formatTanggal(selectedRow.waktu)}</p>
                <p><b>Kasir:</b> {selectedRow.kasir}</p>
                <p><b>Metode:</b> {selectedRow.metode}</p>
                <p><b>Kondisi:</b> {selectedRow.kondisi}</p>
              </div>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {(selectedRow.items || []).map((item: any, i: number) => (
                    <div key={i} className="flex justify-between bg-gray-50 p-3 rounded-lg">
                      <div>
                        <p className="font-semibold text-sm">{item.nama_menu || item.nama}</p>
